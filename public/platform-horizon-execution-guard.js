(function bootstrapHorizonExecutionGuard(root){
  "use strict";
  if(root.CTE_HORIZON_EXECUTION_GUARD)return;

  function loadScript(src,globalKey){
    if(root[globalKey])return Promise.resolve();
    return new Promise((resolve,reject)=>{
      const existing=[...document.scripts].find(script=>script.src?.endsWith(src));
      if(existing){existing.addEventListener("load",resolve,{once:true});existing.addEventListener("error",()=>reject(new Error(`Failed to load ${src}`)),{once:true});return;}
      const script=document.createElement("script");script.src=src;script.async=false;script.addEventListener("load",resolve,{once:true});script.addEventListener("error",()=>reject(new Error(`Failed to load ${src}`)),{once:true});document.head.append(script);
    });
  }

  async function install(){
    await loadScript("/horizon-signal-envelope.js","CTE_HORIZON_SIGNAL_ENVELOPE");
    await loadScript("/horizon-signal-publisher.js","CTE_HORIZON_SIGNAL_PUBLISHER");
    const E=root.CTE_HORIZON_SIGNAL_ENVELOPE,publisher=root.CTE_HORIZON_SIGNAL_PUBLISHER,platform=root.CTE_HORIZON_PLATFORM;
    if(!E||!publisher||!platform)throw new Error("Horizon signal boundary did not initialize");
  ...
  void install().catch(error=>console.error("Horizon execution boundary initialization failed:",error));
})(globalThis);

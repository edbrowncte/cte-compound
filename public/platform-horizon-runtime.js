(function installPlatformHorizonRuntime(root){
  ...
  function renderParity(){const details=ensureParityDisclosure(),target=byId("horizonParityGrid");if(!details||!target)return;const record=currentOptimizerRecord(),status=record?.calculationVersion===VERSION?"IDENTICAL CANONICAL REVISED HTL GEOMETRY":"AWAITING CURRENT COMPUTE CONFIGURATION",cards=[
    ["Canonical crossing",`${VERSION} · completed-candle Asset / direct-WMA-mirror Asset Inverse crossover and crossunder`],
    ...
    ["Effective record",record?`${record.source||"SERVER"} · ${record.range?.bars||"—"} bars · ${record.computedAt?new Date(record.computedAt).toLocaleString():"—"}`:"No current calculation-version record loaded"]
  ];target.innerHTML=cards.map(([label,value])=>`<div class="parity-card"><span>${escape(label)}</span><strong>${escape(value)}</strong></div>`).join("");}

  function ensureTradeManagement(){let panel=byId("platformTradeManagement");if(panel)return panel;const automation=byId("automationPanel");if(!automation)return null;panel=document.createElement("div");panel.id="platformTradeManagement";panel.className="trade-management";panel.innerHTML=`
    ...
    <div class="trade-management-status" id="tradeManagementStatus" role="status" aria-live="polite"><span class="horizon-version">${VERSION}</span></div>`;const candidateStrip=byId("decisionCandidateStrip");if(candidateStrip)candidateStrip.insertAdjacentElement("beforebegin",panel);else automation.append(panel);byId("managedTrade").addEventListener("change",renderManagedTrade);byId("modifyOpenTrade").addEventListener("click",()=>submitTradeAction("MODIFY"));byId("closeOpenTrade").addEventListener("click",()=>submitTradeAction("CLOSE"));return panel;}
  ...
  function augmentDiagnostic(){const grid=byId("platformDiagnosticGrid");if(!grid||grid.querySelector('[data-horizon-version]'))return;const card=document.createElement("div");card.className="diagnostic-card good";card.dataset.horizonVersion=VERSION;card.innerHTML=`<span>Canonical revised HTL geometry</span><strong>${escape(VERSION)} · platform wide</strong>`;grid.append(card);}
  function bind(){installStyles();ensureTradeManagement();renderIdentities();renderParity();byId("downloadTradingLedger")?.addEventListener("click",event=>{downloadForensicLedger(event).catch(error=>{const status=byId("automationStatus");if(status)status.textContent=error.message||"Ledger download failed.";});},true);for(const id of ["chartPair","chartTimeframe","chartStrategy","eventPair","eventTimeframe","eventStrategy","eventLength","engineTimeframe","engineStrategy","engineHtlLength","engineFilter"]){byId(id)?.addEventListener("change",()=>{renderIdentities();renderParity();});}byId("runPlatformDiagnostic")?.addEventListener("click",()=>setTimeout(augmentDiagnostic,250));setInterval(()=>{renderIdentities();renderParity();augmentDiagnostic();if(typeof state!=="undefined"&&state.connected)void loadOpenTrades();},10000);if(typeof state!=="undefined"&&state.connected)void loadOpenTrades();}

  root.CTE_HORIZON_PLATFORM=Object.freeze({VERSION,LENGTH_GRID,FILTER_GRID,canonicalBuild,canonicalEventFeatures,effectiveConfig,renderIdentities,renderParity,loadOpenTrades,submitTradeAction,FORENSIC_FIELDS});
  bind();
})(globalThis);
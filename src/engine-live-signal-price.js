...
export class HtlEngine extends SignalProvenanceEngine{
  ...
  async tickTicket(state,ticket,token,accountId){
    ...
    if(retrogradeEvent(runtime,event)){
      state.lastNoOrderReason=`IO Ticket ${ticket.slot} ignored retrograde ${ticket.pair} ${ticket.timeframe} ${ticket.indicator} event ${event?.startTime||event?.crossingTime||"unknown"}; accepted state remains ${runtime.lastAcceptedEventStartTime||"unknown"}.`;
      ...
      await this.write({type:"INDICATOR_ONLY_RETROGRADE_EVENT_REJECTED",executionPolicy:IO_EXECUTION_POLICY_VERSION,eventChronologyVersion:IO_EVENT_CHRONOLOGY_VERSION,pair:ticket.pair,timeframe:ticket.timeframe,strategy:ticket.indicator,indicatorOnlyTicket:ticket.slot,event:event?.id||null,eventStartTime:event?.startTime||event?.crossingTime||null,lastAcceptedEventId:runtime.lastAcceptedEventId||null,lastAcceptedEventStartTime:runtime.lastAcceptedEventStartTime||null,message:state.lastNoOrderReason},false);await this.ctx.storage.put("state",state);return;
    }
...
export const __liveSignalPriceTest=Object.freeze({LIVE_SIGNAL_PRICE_VERSION,AUTOMATIC_SIGNAL_EXECUTION_VERSION,IO_EVENT_CHRONOLOGY_VERSION,IO_EXECUTION_POLICY_VERSION,PRICE_BASIS,IO_MARKET_SCAN_MAX_MS,IO_STATE_HISTORY_BARS,SIGNAL_ATTEMPT_LIMIT,executableSignalQuote,deduplicateLedgerPayload,quoteUnavailable,ioMarketScanCadenceMs,eventObservedAt,eventStartMs,acceptedEventStartMs,retrogradeEvent,acceptEvent,signalAttemptId,priorSignalAttempt,beginSignalAttempt,finishSignalAttempt});

...
export function crossingSignals(left=[],right=[],source="IOI"){
  ...
  for(let index=1;index<Math.max(left.length||0,right.length||0);index++){
    ...
    if(direction)signals.push({signalIndex:index,sourceIndex:index,direction,source:`${source}_CROSS`});
  }
  return signals;
}
...
export function optimizeIoiIomPerformance(rawCandles,pair,timeframe,lengthGrid,validation,strategyEngineVersion){
  const lengths=[...new Set((lengthGrid||[10,20,30,40,50]).map(value=>Math.max(3,Math.min(500,Math.trunc(Number(value)||10)))))] ,best={IOI:null,IOM:null};
  for(const length of lengths){
    const result=evaluateIoiIomPerformance(rawCandles,pair,length);
  ...
  for(const strategy of IOI_IOM_STRATEGIES){const selected=best[strategy],final=evaluateIoiIomPerformance(normalized,pair,selected.length)[strategy],label=strategy==="IOI"?"IOI · Indicator Only Indicator":"IOM · Indicator Only Mean";config[strategy]={...entry(strategy,selected.length,final.stats,validation,strategyEngineVersion),candidateLengths:[...lengths]};rows.push(exportRow(strategy,label,final.stats,pair,timeframe,selected.length,normalized.length,normalized.at(-1)?.time||null));}

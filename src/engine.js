...
function strategyScore(data,events,pair){const pip=pair.endsWith("JPY")?100:10000,trades=[];for(let index=0;index<events.length-1;index++){const current=events[index],next=events[index+1],entryIndex=current.index+1,exitIndex=next.index+1;if(entryIndex>=data.length||exitIndex>=data.length||exitIndex<=entryIndex)continue;trades.push((data[exitIndex].open-data[entryIndex].open)*current.direction*pip);}let equity=0,peak=0,maxDrawdown=0;for(const net of trades){equity+=net;peak=Math.max(peak,equity);maxDrawdown=Math.max(maxDrawdown,peak-equity);}return{trades:trades.length,net:trades.reduce((sum,value)=>sum+value,0),maxDrawdown,score:trades.length>=2?trades.reduce((sum,value)=>sum+value,0)-.25*maxDrawdown:-Infinity};}
function optimizeDataset(data,pair){const lengths=[10,20,30,50].filter(length=>length*3<data.length),filters={ASSET:[0],DARE_N:[0,.5,1,2],DARE:[0],COMBO:[0,.5,1,2],NAI:[0,.5,1,2],APEX:[0,1,2]},config={};for(const strategy of STRATEGIES){let best={length:lengths[0]||10,filter:filters[strategy][0],score:-Infinity,trades:0,net:0,maxDrawdown:0};for(const length of lengths)for(const filter of filters[strategy]){const stats=strategyScore(data,strategyEvents(data,length,strategy,filter),pair);if(stats.score>best.score)best={length,filter,...stats};}config[strategy]=best;}return config;}
...
export class HtlEngine{
  ...
  async fetch(request){const path=new URL(request.url).pathname;if(path==="/status")return response(await this.status());if(path==="/config"&&request.method==="GET")return response(await this.config());if(path==="/config"&&request.method==="PUT")return response(await this.configure(await request.json()));if(path==="/optimizer"&&request.method==="GET")return response({records:(await this.ctx.storage.get("optimizer"))||{}});if(path==="/optimizer"&&request.method==="PUT")return response(await this.saveOptimizer(await request.json()));if(path==="/ledger")return response({ledger:(await this.ctx.storage.get("ledger"))||[]});if(path==="/tick"&&request.method==="POST"){await this.tick();return response(await this.status());}return response({error:"Not found"},404);}
  ...
  async configure(value){const state=(await this.ctx.storage.get("state"))||{events:{}},prior=normalizeConfig(state.config),next=normalizeConfig(value);if(prior.timeframe!==next.timeframe||prior.htlLength!==next.htlLength||prior.strategy!==next.strategy||prior.confirmationStrategy!==next.confirmationStrategy||prior.filter!==next.filter){state.events={};state.directions=null;state.lastCandle=null;state.initialized=false;}state.config=next;await this.ctx.storage.put("state",state);await this.write({type:"CONFIGURATION",strategy:next.strategy,message:`${next.timeframe} · ${next.strategy} · length ${next.htlLength} · filter ${next.filter} · ${next.decisionMode} · ${next.configurationSource}`});return next;}
  async status(){const state=(await this.ctx.storage.get("state"))||{},config=normalizeConfig(state.config),optimizer=(await this.ctx.storage.get("optimizer"))||{};return{armed:true,running:this.running,...config,triggerMode:"new-completed-candle-only",positionManagement:"all-open-positions",mtfCoverage:Object.keys(state.mtf||{}).length,optimizerCoverage:Object.keys(optimizer).length,optimizerTotal:PAIRS.length*MTF_TIMEFRAMES.length,optimizerCycleIndex:Number(state.optimizerCycleIndex||0),optimizerLastDataset:state.optimizerLastDataset||null,optimizerLastRun:state.optimizerLastRun||null,optimizerLastError:state.optimizerLastError||null,lastCandle:state.lastCandle||null,lastRun:state.lastRun||null,lastError:state.lastError||null,processedPairs:Object.keys(state.events||{}).length};}
  ...
  async saveOptimizer(value){const key=String(value.key||"");if(!/^[A-Z]{3}_[A-Z]{3}\|(W|D|H4|H1|M30|M15|M5|M1|S30|S5)$/.test(key))throw new Error("Invalid optimizer key");const records=(await this.ctx.storage.get("optimizer"))||{};records[key]={stamp:value.stamp||new Date().toISOString(),config:value.config||{}};await this.ctx.storage.put("optimizer",records);return records[key];}
  pairConfig(config,optimizer,pair,timeframe){if(config.configurationSource!=="OPTIMIZED")return config;const entry=optimizer?.[`${pair}|${timeframe}`]?.config?.[config.strategy];if(!entry)return config;return{...config,htlLength:Math.max(3,Math.min(200,Math.trunc(Number(entry.length))||config.htlLength)),filter:Math.max(0,Math.min(10,Number(entry.filter)||0))};}
  async optimizeNext(state,token){const total=PAIRS.length*MTF_TIMEFRAMES.length,index=Number(state.optimizerCycleIndex||0)%total,pair=PAIRS[index%PAIRS.length],timeframe=MTF_TIMEFRAMES[Math.floor(index/PAIRS.length)],data=await candles(pair,token,timeframe),config=optimizeDataset(data,pair),stamp=data.at(-1)?.time||new Date().toISOString(),records=(await this.ctx.storage.get("optimizer"))||{};records[`${pair}|${timeframe}`]={stamp,computedAt:new Date().toISOString(),source:"SERVER",config};await this.ctx.storage.put("optimizer",records);state.optimizerCycleIndex=(index+1)%total;state.optimizerLastDataset=`${pair}|${timeframe}`;state.optimizerLastRun=new Date().toISOString();state.optimizerLastError=null;}
  async scan(token,config,timeframe=config.timeframe,optimizer={}){const rows=[];for(const pair of PAIRS){const effective=this.pairConfig(config,optimizer,pair,timeframe),data=await candles(pair,token,timeframe),event=currentEvent(data,effective.htlLength,effective.strategy,effective.filter),confirmation=effective.confirmationStrategy==="NONE"?null:currentEvent(data,effective.htlLength,effective.confirmationStrategy,effective.filter);if(event&&(!confirmation||confirmation.direction===event.direction))rows.push({pair,event,configuration:{length:effective.htlLength,filter:effective.filter}});}return rows;}
  async reconcile(directions,token,accountId){
    ...
    for(const position of positions){
      ...
      await this.write({type:"POSITION_CLOSED",pair:position.instrument,strategy:(await this.config()).strategy,direction:existing>0?"BUY":"SELL",units:existing>0?longUnits:shortUnits,message:"Position opposed current strategy direction"});
    }
  }
  ...
  async tick(){
    ...
    try{
      const config=normalizeConfig(state.config);state.config=config;const optimizer=(await this.ctx.storage.get("optimizer"))||{}, {token,configured}=secrets(this.env),accountId=await liveAccount(token,configured);try{await this.optimizeNext(state,token);}catch(error){state.optimizerLastError=error.message||"Optimizer failure";}const rotationIndex=Number(state.mtfRotation||0)%MTF_TIMEFRAMES.length,rotationTimeframe=MTF_TIMEFRAMES[rotationIndex],rotationRows=await this.scan(token,config,rotationTimeframe,optimizer);state.mtf=state.mtf||{};state.mtf[rotationTimeframe]={directions:Object.fromEntries(rotationRows.map(row=>[row.pair,row.event.direction])),updated:new Date().toISOString()};state.mtfRotation=(rotationIndex+1)%MTF_TIMEFRAMES.length;const probe=await candles("EUR_USD",token,config.timeframe,2),lastCandle=probe.at(-1)?.time;
  ...
  async execute(candidate,token,accountId,state){
    ...
    if(existing){const body=existing>0?{longUnits:"ALL"}:{shortUnits:"ALL"},closed=await callOanda(`/v3/accounts/${accountId}/positions/${pair}/close`,token,{method:"PUT",body:JSON.stringify(body)}),fill=closed.longOrderFillTransaction||closed.shortOrderFillTransaction||{},closedUnits=Math.abs(Number(fill.units)||(existing>0?longUnits:shortUnits));await this.write({type:"POSITION_CLOSED",pair,strategy:normalizeConfig(state.config).strategy,direction:existing>0?"BUY":"SELL",units:closedUnits,price:fill.price||null,realizedPL:fill.pl??null,message:"Opposite strategy event"});}
    ...
    await this.write({type:"ORDER_FILLED",pair,strategy:normalizeConfig(state.config).strategy,direction,units,transaction:fill?.id||result.lastTransactionID||null,price:fill?.price||null,event:event.id});
  }
}

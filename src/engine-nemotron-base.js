...
export class HtlEngine extends HorizonEngine {
  ...
  async choose(candidates){
    ...
    try{
      ...
      await this.recordAiDecision({invoked:true,status,reason,model:AI_MODEL,policy:AI_POLICY,taskName:AI_TASK_NAME,task:AI_TASK,latencyMs,candidateCount:candidates.length,candidates:candidatePairs,selectedPair:fallback.pair}).catch(()=>{});

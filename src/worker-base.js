...
async function handlePlatformDiagnostic(env,url){
  ...
  const optimizer=engine.ok?await diagnosticStep("OPTIMIZER_PERSISTENCE",async()=>{const value=engine.value||{},error=String(value.optimizerLastError||"").trim();if(error||value.optimizerPersistenceHealthy===false)throw decorateError(new Error(error||"Optimizer persistence reported unhealthy."),{status:503,code:"OPTIMIZER_PERSISTENCE_FAILURE",stage:"OPTIMIZER_PERSISTENCE",retryable:true});if(value.optimizerStorageMode!=="SHARDED_PER_DATASET")throw decorateError(new Error(`Unexpected optimizer storage mode: ${value.optimizerStorageMode||"unreported"}.`),{status:503,code:"OPTIMIZER_STORAGE_MODE_INVALID",stage:"OPTIMIZER_PERSISTENCE",retryable:false});return{healthy:true,storageMode:value.optimizerStorageMode,coverage:Number(value.optimizerCoverage||0),total:Number(value.optimizerTotal||0),lastRun:value.optimizerLastRun||null,lastDataset:value.optimizerLastDataset||null};}):skippedDiagnostic("OPTIMIZER_PERSISTENCE","Engine status unavailable.");
  const required=[credentialCheck,accountList,summary,candles,engine,optimizer],failures=required.filter(step=>!step.ok),tradingCritical=[credentialCheck,accountList,summary,candles],verdict=tradingCritical.some(step=>!step.ok)?"FAIL":engine.ok&&optimizer.ok?"PASS":"DEGRADED",firstFailure=failures.find(step=>!step.skipped)||failures[0]||null;
  return json({verdict,time:new Date().toISOString(),totalLatencyMs:Date.now()-started,deployment:deploymentMetadata(env),checks:{credentials:credentialCheck,accountList,summary,candles,engine,optimizer},worker:{oandaActive,oandaQueued:0,maxConcurrency:null,concurrencyMode:OANDA_CONCURRENCY_MODE,requestTimeoutMs:OANDA_REQUEST_TIMEOUT_MS,candleCacheEntries:candleCache.size,candleCacheBars:candleCacheBarCount(),candleCacheMaxEntries:CANDLE_CACHE_MAX_ENTRIES,candleCacheMaxBars:CANDLE_CACHE_MAX_BARS,telemetry:oandaTelemetry},oanda:{accountSuffix:resolvedAccountId?.slice(-3)||credentialValue?.accountId?.slice(-3)||null,intendedAccountVisible:accountVisible,summaryLatencyMs:summary.latencyMs,candleLatencyMs:candles.latencyMs,completedCandles:candles.value?.completedCandles||0,NAV:summary.value?.NAV||null,marginAvailable:summary.value?.marginAvailable||null,failure:firstFailure&&!firstFailure.stage.startsWith("ENGINE")&&!firstFailure.stage.startsWith("OPTIMIZER")?firstFailure:null},engine:engine.ok?{reachable:true,...engine.value}:{reachable:false,lastError:engine.error,code:engine.code,diagnosticId:engine.diagnosticId},failure:firstFailure?{stage:firstFailure.stage,code:firstFailure.code,error:firstFailure.error,status:firstFailure.status,retryable:firstFailure.retryable,diagnosticId:firstFailure.diagnosticId}:null,cloneAssessment:{structuredCloneCalls:0,applicable:false,verdict:"No structuredClone hot path exists in this repository."}});
}

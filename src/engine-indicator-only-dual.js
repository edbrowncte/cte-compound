...
export class HtlEngine extends IndicatorOnlyUnitsEngine{
  async fetch(request){
    ...
    if(path==="/control/status"&&request.method==="GET"){
      ...
      return response({...payload,...view},parent.status);
    }
  ...
  async status(){
    ...
    return{...parent,...indicatorOnlyStatus(state)};
  }
  ...
  async tickTicket(state,ticket,token,accountId){
    ...
    if(existing){const longUnits=Number(position?.long?.units||0),shortUnits=Math.abs(Number(position?.short?.units||0)),fill=await this.closePosition(ticket.pair,existing,longUnits,shortUnits,token,accountId,executionEventId,`IO Ticket ${ticket.slot} opposing new indicator signal reversal`,context);if(!fill){state.lastNoOrderReason=`IO Ticket ${ticket.slot} reversal close failed for ${ticket.pair}`;return;}}
    const fill=await this.executeIndicatorOnlyUnits(candidate,token,accountId,state);if(fill){const executionAt=Date.now(),observedTime=Date.parse(observedAt||"");runtime.lastExecutionEventId=executionEventId;runtime.lastExecutionAt=new Date(executionAt).toISOString();runtime.lastExecutionUnits=ticket.units;runtime.executionDelayMs=Number.isFinite(observedTime)?Math.max(0,executionAt-observedTime):null;await this.write({type:"INDICATOR_ONLY_EXECUTION_TIMING",executionPolicy:IO_DUAL_VERSION,pair:ticket.pair,direction:event.direction>0?"BUY":"SELL",timeframe:ticket.timeframe,strategy:ticket.indicator,indicatorOnlyTicket:ticket.slot,event:executionEventId,eventStartTime:event.startTime||null,eventObservedAt:observedAt,executionAt:runtime.lastExecutionAt,executionDelayMs:runtime.executionDelayMs,message:`IO Ticket ${ticket.slot} execution delay ${runtime.executionDelayMs??"unknown"} ms from new-event observation`},false);}
  }

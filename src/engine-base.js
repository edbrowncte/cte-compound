...
export class HtlEngine{
  ...
  async write(entry,sendNotification=true){const record={...entry,time:new Date().toISOString(),ledgerId:crypto.randomUUID()},key=`ledger:${record.time}:${record.ledgerId}`,index=(await this.ctx.storage.get("ledgerIndex"))||[];await this.ctx.storage.put(key,record);index.unshift(key);const removed=index.splice(5000);await this.ctx.storage.put("ledgerIndex",index);if(removed.length)await this.ctx.storage.delete(removed);const legacy=(await this.ctx.storage.get("ledger"))||[];legacy.unshift(record);await this.ctx.storage.put("ledger",legacy.slice(0,500));if(sendNotification)await notify(this.env,record);}

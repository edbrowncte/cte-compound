...
replaceOnce(
  "Oscillator heading",
  "Synced MAS/IM Oscillator",
  "Synced MAS / IM Pressure"
);
...
replaceOnce("Evaluation default sort",'    evalSortKey:"strength",','    evalSortKey:"transitionProbability",');
replaceOnce("Remove obsolete Evaluation slope state",'    evaluationSlopeHistory:{},\n','');
replaceOnce(
...
const calculatorPath="public/mas-im-calculator.js";
let calculator=await readFile(calculatorPath,"utf8");
const bug='if(!(hours>0)&&!Number.isFinite(hours))';
if(!calculator.includes(bug))throw new Error("event velocity hours guard marker not found");
calculator=calculator.replace(bug,'if(!(hours>0)||!Number.isFinite(hours))');
await writeFile(calculatorPath,calculator);

console.log("Applied MAS antagonist pressure v2 Evaluation UI and calculator guard migration");

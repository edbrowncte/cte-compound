...
assert.equal(api.VERSION,"CTE_HTL_SIGNAL_PANEL@1.0.1");
assert.ok(Math.abs(api.directionalGain("EUR_USD",1.1050,1.1000,1)-50)<1e-9);
assert.ok(Math.abs(api.directionalGain("EUR_USD",1.0950,1.1000,-1)-50)<1e-9);
...
assert.equal(api.directionalGain("EUR_USD",1.1,"",1),null);
...
assert.match(source,/function normalizeFromTwoExports/);assert.match(source,/s\.scheduleEvaluations/);assert.match(source,/s\.eventRows/);assert.doesNotMatch(source,/fetch\(["']\/api\/engine\/status/,"panel must not depend on engine status for analytical rows");
assert.match(source,/eventTimeframe===timeframe\?\(s\.eventRows\|\|\[\]\)/,"panel must never relabel HTL Schedule rows from another timeframe");
assert.match(source,/function scheduleIndicatorIds\(\)/);assert.match(source,/document\.getElementById\("scheduleStrategy"\)\?\.options/,"panel indicator selector must mirror the actual Timeframe Signal Schedule registry");
...
console.log("HTL signal alignment panel certification passed: frontend state/export join, timeframe isolation, real schedule-indicator registry, directional gain, sortable headers/arrows, gain coloring, Analytical Compound placement, and JSON export are wired without a new backend endpoint.");

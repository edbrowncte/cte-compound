...
response=await worker.fetch(request,{ASSETS:assets,CTE_RELEASE_ENFORCEMENT:"ENFORCE_CURRENT_RELEASE",CTE_RELEASE_CONTRACT:"CTE_COMPOUND_CURRENT_RELEASE@2.0.0",CTE_RELEASE_SHA:"a".repeat(40)});
assert.equal(response.status,200);assert.equal(await response.text(),"current");
response=await worker.fetch(request,{ASSETS:assets,CTE_RELEASE_ENFORCEMENT:"ENFORCE_CURRENT_RELEASE",CTE_RELEASE_CONTRACT:"CTE_COMPOUND_CURRENT_RELEASE@2.0.0",CF_VERSION_METADATA:{id:"21e019cb-fd70-48b0-87f0-a4d4a3285bab"}});
assert.equal(response.status,200);assert.equal(await response.text(),"current");

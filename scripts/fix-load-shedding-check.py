...
test = replace_once(test, 'assert.match(html,/await loadTradeCapacity\\(\\);void loadChart\\(\\);void loadSchedule\\(\\);/);', 'assert.match(html,/await loadTradeCapacity\\(\\);await loadChart\\(\\);void loadSchedule\\("focused"\\)/);', "foreground-first startup contract")

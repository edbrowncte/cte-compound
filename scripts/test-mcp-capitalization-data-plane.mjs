...
assert.match(client,/function applyPairVisibility\(\)/);assert.match(client,/row\.hidden=Boolean\(label&&text!==label\)/,"Pair focus must be a display filter over the rendered tables.");assert.doesNotMatch(client,/state\.evaluationTableData\s*=\s*original\.filter/,"Pair focus must never shrink or replace the canonical 28-pair Evaluation snapshot.");
...
console.log("MCP Capitalization certification passed: Evaluation Table and Rate Fluctuation load independently of schedule coverage, expose synchronized analytical-only currency-pair/timeframe controls, preserve the full 28-pair snapshot and universe ranks, restore the four forecast cards, and retain server-side MAS/IM data authority.");
